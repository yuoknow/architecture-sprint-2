...
OUTPUT=$(docker compose exec -T shard1 mongosh --quiet --eval "use somedb" --eval "db.helloDoc.countDocuments()" --port 27018)
echo "Docs on shard1: ${OUTPUT}"

OUTPUT=$(docker compose exec -T shard2 mongosh --quiet --eval "use somedb" --eval "db.helloDoc.countDocuments()" --port 27019)
